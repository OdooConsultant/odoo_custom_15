        ...
        if move_lines:
            for line in move_lines[0]:
                ...
                if self.env['ir.config_parameter'].sudo().get_param('ks_enable_ledger_in_bal'):
                    ...
                    sheet.write_number(row_pos, 5, float(move_lines[0][line].get('initial_balance')), line_header)
                ...
                if ks_df_informations.get('ks_report_with_lines', False):
                    ...
                    for sub_line in move_lines[0][line]['lines']:
                        if sub_line['initial_bal']:
                            ...
                            if self.env['ir.config_parameter'].sudo().get_param('ks_enable_ledger_in_bal'):
                                sheet.write_number(row_pos, 5, float(sub_line.get('initial_balance',0)),
                                                   line_header_light_initial)
                                sheet.write_number(row_pos, 6, float(sub_line.get('debit')),
                                                        line_header_light_initial)
                                sheet.write_number(row_pos, 7, float(sub_line.get('credit')),
                                                        line_header_light_initial)
                                sheet.write_number(row_pos, 8, float(sub_line.get('balance')),
                                                        line_header_light_initial)
                            else:
